...
# M-h4: the deployed app's public URL, for `make deploy-smoke`. The committed default is the
...
fly-egress-check: ## M-h3 LIVE gate (the EGRESS fence): boot TWO real microVMs — one WITHOUT the network policy (the reproduce-first control) and one via the real `start_run_sandbox` WITH it — and socket-probe the SAME literal IPs from inside each. Asserts the DELTA: a port with a real listener that was OPEN unfenced (github:22 / gmail-smtp:25) goes TIMEOUT fenced, while the allowlisted :443 and :53 stay OPEN and the agent server is still reachable over Flycast (Fly-Proxy traffic is policy-exempt). Distinguishes REFUSED (reached the host, nothing listening — NOT fence evidence) from TIMEOUT (dropped). Reports boot/ready seconds for both legs. No agent, no model call — minutes and cents. Both apps torn down in a `finally`. Skips cleanly without TVASHTR_FLY_API_TOKEN. Needs an ACTIVE WireGuard tunnel. NOT in `make test`.
	cd backend && TVASHTR_AGENT_SANDBOX=fly uv run python ../scripts/fly_egress_check.py
...
deploy-smoke: ## M-h4 LIVE smoke against the DEPLOYED app (default https://tvashtr.fly.dev; override TVASHTR_DEPLOY_URL): asserts /health 200 {"status":"ok","db":"ok"} (Neon reachable + the release_command's schema present), the DB-free /healthz 200, /api/config hosted_mode:true with a github_install_url carrying a redirect_uri for THIS deployment, / and a client-route deep link both serving index.html (one-origin SPA + the catch-all), an unknown /api path still 404-ing as JSON (the catch-all never shadows the API), and http:// redirecting to https (force_https). Read-only, credential-free, no Fly API calls — safe to re-run. The first request pays a cold start (the machine sleeps when idle). NOT in `make test`.
	cd backend && uv run python ../scripts/deploy_smoke_check.py --base-url $(TVASHTR_DEPLOY_URL)

demo-proof: ## M-proof LIVE gate (LOCAL leg, PERMANENT — never deleted after use): drive the whole demo path through a REAL browser against the local stack in HOSTED mode on the LOCAL sandbox — sign in, assert >=2 provider keys incl. nvidia_nim, create a `PM -> Engineer <-> Reviewer` team, assert each model chip reads ITS SEAT's default from the first held provider that serves that seat (derived from GET /api/config — nvidia_nim serves no seat since 2026-09-26; never openrouter/), the account-derived `fallback_model` on ALL THREE model-bearing nodes, caveman on the two WORKERS and NOT on the thinker, then launch at a real GitHub repo (TVASHTR_PROOF_REPO, default lazyxgenius/trade_mcp), APPROVE every human gate by CLICKING the real Approve button (never the API, never a forced reviewer verdict), and assert a terminal run with a REAL non-empty pr_url — one screenshot per check under artifacts/demo-proof/local/. Needs GITHUB_APP_* + >=2 provider keys; skips cleanly without the App, stops NEEDS_HUMAN on the provider bar. NOT in `make test`.
	./scripts/demo_proof.sh local

demo-proof-prod: ## M-proof LIVE gate (PROD leg, PERMANENT): the SAME C1-C12 harness against the DEPLOYED app ($(TVASHTR_DEPLOY_URL)) using the session saved by `make demo-proof-login`. Mutates NO prod configuration — no installation rows, no provider keys — and opens a REAL pull request from production. Stops NEEDS_HUMAN (with the exact remedy) when the saved session is missing or stale. Screenshots under artifacts/demo-proof/prod/. NOT in `make test`.
	./scripts/demo_proof.sh prod

demo-proof-login: ## M-proof one-off: open a HEADED browser at $(TVASHTR_DEPLOY_URL) so the operator can finish the GitHub OAuth by hand; Playwright then saves the resulting session to a GITIGNORED storageState file for `make demo-proof-prod`. The prod account holds auth.UNUSABLE_PASSWORD_HASH (created via the OAuth callback), so /api/auth/login can never succeed for it — this is the only honest way in, and github.com is never automated. The session lasts ~14 days. NOT in `make test`.
	./scripts/demo_proof.sh login
...
workspace-gc-check: ## M-wsgc gate (orphaned agent-workspace reaper, DELIVERABLE-AWARE): seed REAL .tvashtr_workspaces/<run_id> dirs for a terminal BROWNFIELD run, an ABSENT run (no Run row), a terminal GREENFIELD run and a PARKED (awaiting_human) run — each at the EXECUTOR's own `make_local_workspace` path, so the gate FAILS if the reaper's replicated WORKSPACE_ROOT ever drifts from the one runs are written to -> run sweep_orphaned_workspaces() -> assert the brownfield worktree + the orphan are REAPED while the GREENFIELD workspace (it IS the run's deliverable — shipped commit+tag, no remote) and the parked run's workspace SURVIVE. Credential-free and DOCKER-FREE: pure local FS + Postgres, no container, no Fly/GitHub/model keys. Fixtures torn down in a `finally`. NOT in `make test`.
	cd backend && uv run python ../scripts/workspace_gc_check.py

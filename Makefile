...
memory-review-gate: ## M-memory S4 LIVE gate: on a REAL LOCAL-sandbox deepseek run proves the write-control surface end-to-end — (a) an agent-remember lands active/repo via the FALLBACK .tvashtr/remember.jsonl channel (TVASHTR_MEMORY_REMEMBER_ENABLED=1), (b) review mode ON quarantines a distilled fact pending_review, (c) promote consolidates (activates + retires the contradicted active fact), (d) reject tombstones + suppresses a re-proposal — echoing each step's DB state. Needs DEEPSEEK_API_KEY (agent) + OPENAI_API_KEY (distill+embeds); skips cleanly otherwise. NOT in `make test`.
	cd backend && TVASHTR_AGENT_SANDBOX=local TVASHTR_AUTO_APPROVE_GATES=1 TVASHTR_FORCE_REVISIONS=1 TVASHTR_MEMORY_REMEMBER_ENABLED=1 uv run python ../scripts/memory_review_gate.py

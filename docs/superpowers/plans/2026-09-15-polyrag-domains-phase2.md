...
- **File storage (locked):** filesystem under `settings.domain_files_dir` (env `TVASHTR_DOMAIN_FILES_DIR`, default `/tmp/tvashtr-domain-files` in tests/dev). Path layout `{dir}/{owner_id}/{domain_id}/{document_id}/{safe_filename}`. **No Tigris/S3 in Phase 2.** **Prod (Fly multi-replica):** require a shared volume and set `TVASHTR_DOMAIN_FILES_DIR=/data/domain-files` so upload and ingest share the same bytes; `/tmp` is not durable across restarts or machines.

...
**Optional pre-P1.8 housekeeping (operator's call):** the CLI-RULES staleness (below, §6) — a fuller refresh of `prompts/CLI-RULES.md` §7 (its milestone sequence still lists finished P1.5c) before P1.8's first `/goal`. Not blocking — the P1.8 brief will supersede §7 for its run, as the FE-infra brief did.

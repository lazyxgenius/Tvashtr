#!/usr/bin/env bash
#
# Live FE E2E for Mode A ("Ask the node"). Registers a fresh account, seeds its deepseek key, creates
# a review_loop team from the template, and drives a REAL run to completion in the LOCAL sandbox on
# deepseek/deepseek-chat with a FORCED reviewer-approve (TVASHTR_FORCE_REVISIONS=0 -> the reviewer
# short-circuits to "approved", makes NO LLM call; only the PM + Engineer call deepseek; NO docker
# agent containers). Then it opens the run view, clicks a node, opens "Ask", asks a question, and
# asserts a NON-EMPTY answer renders — screenshotting each check.
#
# Orchestration mirrors scripts/run_diff_e2e.sh (Postgres + migrate + backend + Vite + headless
...
BACKEND_LOG="/tmp/tvashtr_node_ask_backend.log"
VITE_LOG="/tmp/tvashtr_node_ask_vite.log"
export TVASHTR_NODE_ASK_SHOTS_DIR="${TVASHTR_NODE_ASK_SHOTS_DIR:-/tmp/tvashtr_node_ask_shots}"
...
if [[ -z "${DEEPSEEK_API_KEY:-}" ]]; then
  echo "[node-ask-e2e] DEEPSEEK_API_KEY not set — skipping. (Not a failure.)"
  exit 0
fi
...
hr
echo "STEP F: run the Mode A node-ask Playwright spec"
hr
cd "$FRONTEND"
set +e
TVASHTR_E2E_BASE_URL="http://127.0.0.1:${VITE_PORT}" npx playwright test e2e/node-ask.spec.ts
PW_EXIT=$?
set -e

hr
if [[ "$PW_EXIT" == "0" ]]; then
  echo "NODE-ASK E2E PASSED (real run shipped on deepseek; the Ask tab renders a non-empty answer)"
  echo "screenshots: $TVASHTR_NODE_ASK_SHOTS_DIR"
  ls -la "$TVASHTR_NODE_ASK_SHOTS_DIR"/*.png 2>/dev/null || true
else
  echo "NODE-ASK E2E FAILED at the Playwright proof (exit $PW_EXIT)"
fi
hr
exit "$PW_EXIT"

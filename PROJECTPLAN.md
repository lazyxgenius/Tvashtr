...
>
> **Last updated:** 2026-06-15 by **Tvashtr-4** — *Phase 0 **COMPLETE & CLOSED**. P0.5 (minimal UI) shipped and verified: the proven resumable spine is now **visible** (React Flow canvas, live per-node status) and **inspectable** (PM's PRD + versions, Engineer's run-event feed), all on the operator's Design System. **Milestone M0 fully achieved.** Phase 0's five steps (P0.1–P0.5) are done; the walking skeleton is complete end-to-end. **Handing to Tvashtr-5 to begin Phase 1 (the painkiller: Supervisor MCQ scoping + a real default team).** See HANDOVER.md + §15/§16/§17.*
>
...
- **Cancel / abort a run from the UI** *(from P0.5a, 2026-06-15)*. A killed-mid-run `run_team` is *resumed* by DBOS on the next backend launch (the M0 crash-recovery property) — so there is currently no way to *intentionally* stop a run; start-and-abandon resurrects it. Add a 'cancel run' affordance (DBOS programmatic cancel — the kill-switch path already named for Risk 3). *Trigger:* when runs are long/expensive enough that an unwanted resume matters, or alongside the kill-switch / cost-cap work (Phase 1).

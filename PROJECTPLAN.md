...
- **Grounding worker-protocol reaches reviewer nodes** *(carry-forward — resolve before the first brownfield review_loop run)* — the D6 grounding's worker-protocol block ("implement by editing in place…") is appended to ALL agent nodes via `agent_run_step`, including reviewer-style ones, and could mis-steer a reviewer into implementing rather than gating. Untested in Slice 1 (the two_node fixture has no reviewer). *Trigger:* before the first brownfield review_loop run — gate the worker-protocol block to worker (non-reviewer) nodes, split from the always-safe orientation block.
- **Brownfield review_loop run (Reviewer gates the real diff on a real repo)** — Slice 1's automated gate uses `two_node` for speed/robustness; the Reviewer-gating-the-real-diff path is the same machinery on the worktree mount as greenfield M1, but unproven on a real repo. *Trigger:* after Slice 2, as the M-brownfield exit-bar proof (the graduated real-repo ladder, D4).
- **Selective (git-diff) workspace pull** — the brownfield pull copies all non-`.git` container files each iteration (correct via host-side `git add -A`, but O(working-set)); the standard `git_changes`/diff-only pull (seed a container-side git baseline) is faster and the OpenHands-native mechanism. *Trigger:* when a real (large) repo run gets slow — relevant the moment we test beyond a tractable fixture.
...
- **M-brownfield — A real feature-shipping Tvashtr (the active bet, Tvashtr-31):** a composed, human-steered team ships a **correct, reviewed** feature into the user's **real existing repo**, run locally (the cheapest version on the current stack — local-run + repo-mount). This is now the near-term gate that *precedes* demand validation (the operator's build-first stance). Clearing it = differentiated *and* worth validating; not clearing it at solo scale = the most important thing to learn (§1, §13 S4). **Progress (Tvashtr-32):** Slices 1 (backend run mode) + 2 (launch-panel UI) SHIPPED + merged — the worktree mount, branch ship, git-aware sync, repo-grounding, and the run-launch UI (idea + repo target + worker-model hint) are all in. **Remaining to clear the exit bar (Slice 3+):** the brownfield review_loop run (the D6 worker-gating carry-forward) + the graduated real-repo ladder (a real, non-trivial feature on progressively harder real repos).

...
>
> **Last updated:** 2026-06-15 by **Tvashtr-5** — *Phase 0 closed; **Phase 1 decomposed into P1.1–P1.9, sequence LOCKED** (loop-first; §13 safety/cost/control enablers front-loaded — see §15). **P1.1 (HitL foundation: gates + Tasks-for-Human + kill switch) ✅ COMPLETE & independently verified** — P1.1a (durable gate `recv`/`send` + kill switch `cancel_workflow`→`CANCELLED`, inlined PRD-approval gate), P1.1b (Tasks-for-Human banner-card UI + `run.status`-authoritative status derivations + cancel-closes-tasks + vitest stood up), P1.1c (muted "Stopped" node state for reject/cancel + the resolve-flicker fix). Verified by reading every changed file + an operator live visual sign-off across approve→ship / reject / cancel; the live check caught a StrictMode `mountedRef` bug (fixed directly) that all headless gates had passed. `make hitl-demo` green; M0 crash-resume preserved through the gate; vitest 34 + build clean. **Next: P1.2** — cost caps + budget enforcement (the §13 Risk-3 chokepoint, reusing P1.1's interrupt path). See §15/§17 + HANDOVER.md.*
>
> **Last updated (DRAFT — P1.2 build loop, 2026-06-16; architect to verify & finalize):** **P1.2 (cost caps + budget enforcement)** built & loop-verified on `feat/p1.2-cost-caps` (un-merged) — a **per-run dollar cap** enforced in the **Control Plane** (gateway stays a pure metering chokepoint), a breach **pauses as a `budget_approval` blocker** reusing P1.1's gate (approve = continue → completion / reject = new terminal **`over_budget`**, no ship), a per-call `max_tokens` default, and the muted **"Stopped"** node + **"Over budget"** banner (reusing P1.1c visuals, no new CSS). Per-project cap + 80% threshold alert + depth/loop/active-agent ceilings **deferred** (§15). Offline (41 tests) + lint + frontend (build + 37 vitest) + `0007` migration round-trip + browser(mocked) + independent-review gates green. **Operator Human-checks pending:** `make budget-demo` (live), regressions `make skeleton-crash`/`make hitl-demo`, the live `over_budget` visual, and an eyeball of migration `0007`. See §15/§17.
>
...
- **P1.2 — Cost caps + budget enforcement (in-process).** Layered caps with the Model Gateway as the dollar chokepoint: per-call/token caps, per-run + per-project hard-stops + threshold alerts, depth/loop/active-agent ceilings; the hard-stop reuses P1.1's interrupt path. (Risk 3.) **Status (DRAFT — P1.2 build loop 2026-06-16, architect to verify/finalize):** the *live, validatable slice* is **built & loop-verified** on `feat/p1.2-cost-caps` (un-merged) — a **per-run dollar cap** enforced in the **Control Plane** (the gateway stays a pure dollar-*metering* chokepoint), a breach **pauses as a `budget_approval` blocker** reusing P1.1's gate (approve = continue / reject = new terminal `over_budget`), and a per-call `max_tokens` default. Per-project cap + threshold (80%) alert + depth/loop/active-agent ceilings are **deferred** (registered above). Offline (`make test` 41) + lint + frontend (`npm run build`/`npm test` 37) + migration-`0007`-round-trip + browser (mocked over_budget render) + independent-review gates all green; live `make budget-demo` + regressions (`make skeleton-crash`/`make hitl-demo`) + the live over_budget visual are **operator Human-checks**.
- **P1.3 — Docker sandbox** (OpenHands Agent Server, behind the existing `EngineAdapter` seam). Replaces local-unsandboxed execution **before** the loop runs real multi-file / build / test code (the demonstrated write-escape → real containment). Slotted after the kill switch + caps so containment-verification runs are already killable and cost-capped. *Validated against the existing greeting task.*
...
- **Agent-native resume** *(from P0.4 Decision 1, §17)*. On crash recovery, resume a persisted OpenHands `Conversation` instead of restarting the agent from scratch, eliminating wasted-retry spend. *Trigger:* agent runs long/expensive enough that paying twice on a crash hurts, or when the Docker-sandbox seam is built.
...
- **LiteLLM proxy as the physical metering chokepoint** *(already roadmapped, Phase 1; §8/D9)*. All traffic — direct + agent — flows through one metered endpoint, dissolving the agent-metering question entirely.
...
- **Docker-sandboxed engine workspace** *(§13, §15-P0.3; ⚠️ write-escape DEMONSTRATED in P0.4b)*. Replace local-unsandboxed mode with the OpenHands Agent Server Docker sandbox, behind the same `EngineAdapter`. **The escape risk is now concrete, not theoretical:** in P0.4b the agent wrote `greeting.txt` *outside* its workspace when the PM emitted an absolute `/greeting.txt` path (the leaked `backend/greeting.txt` was found + removed; a prompt-level fix now tells the agent to strip a leading `/`/`./` and write relative — a band-aid, NOT containment). Also noted: the OpenHands terminal tool's CWD may differ from the workspace. *Trigger:* before any untrusted-code execution; tracked as the `# TODO(P0.x)` seam in the adapter.
...
- **2026-06-16 (P1.2 build loop — DRAFT, architect to verify/finalize + add the P1.2 *design* entry) — P1.2 (cost caps + budget enforcement) implemented on `feat/p1.2-cost-caps` (un-merged).** Built to the locked DP-A–D design (brief `prompts/P1.2-cost-caps-budget-enforcement.md`).
  ...
  - **Live targets NOT run in-loop** (the agent is still unsandboxed until P1.3): `make budget-demo` (new — `scripts/budget_demo.sh`: tiny cap → pre-ship breach → auto-approve → ships, asserting a `budget_approval` task created+resolved+approved + run shipped; **built, not run**), the regressions `make skeleton-crash`/`make hitl-demo`, and the live `over_budget` visual — all **operator Human-checks** (HALT-and-escalate if a live regression needs an architectural fix, e.g. the new `budget_check_step` perturbs DBOS replay).
  - **Deferred → §15:** richer budget-breach resolution (binary override vs grant-a-specific-increment) — *Trigger:* P1.5.
  - **Un-merged on `feat/p1.2-cost-caps`; the loop commits, the operator merges.** Architect to verify against the files, run the Human-checks, finalize this entry + the §15 roadmap bump + the "Last updated" line, and add the P1.2 *design* entry.

...
- **P1.3 — Docker sandbox** (OpenHands Agent Server, behind the existing `EngineAdapter` seam). Replaces local-unsandboxed execution **before** the loop runs real multi-file / build / test code (the demonstrated write-escape → real containment). Slotted after the kill switch + caps so containment-verification runs are already killable and cost-capped. *Validated against the existing greeting task.* **[DRAFT — Tvashtr-7/P1.3a, 2026-06-16]: P1.3a SHIPPED on `feat/p1.3a-docker-sandbox` (offline-green) — the containerized path (`DockerWorkspace`→`RemoteConversation`, DQ1 pull-at-end, orphan-reaping) behind the *unchanged* `EngineAdapter` contract, selected by `agent_sandbox_mode` (default `local`). The container path itself is pending the operator live check. P1.3b (prove containment + crash-resume over the container, then flip the default to `docker`) is next.]**
...
- **Dynamic/ephemeral host ports + per-run container targeting for concurrent agent runs** *(from P1.3a, 2026-06-16 — Tvashtr-7)*. P1.3a runs the agent-server container on a **fixed** host port (`agent_server_host_port=8010`) and reaps orphans **by image** (`docker … --filter ancestor=<server_image>`), because the installed `DockerWorkspace` (1.28.1) names its container `agent-server-<uuid>` with **no per-run name/label hook** — so reaping is all-or-nothing across runs. Correct while runs are **serial** (single-operator v1). When concurrent agent runs land, two coupled upgrades are needed: ephemeral/auto host ports (the SDK's `host_port=None` finds a free port) so containers don't collide on the port, **and** per-run container targeting (a deterministic name/label) so reaping isolates one run — likely a small fork/patch of `DockerWorkspace` or a newer SDK exposing a name hook. *Trigger:* when >1 agent run can be in flight at once (parallel nodes in P1.5's loop, or multi-project).
...
- **2026-06-16 (Tvashtr-7 — P1.3a; built via the `tvashtr-loop` loop) — DRAFT (for architect verification + finalization). Docker-sandboxed execution path: implemented + OFFLINE-green; the container path itself is PENDING the operator live check.** Built to the locked brief `prompts/P1.3a-containerized-execution-path.md` (DQ1–DQ4). The architect adds the P1.3 *design* entry (DQ1–DQ4 rationale) and finalizes this entry **after** the operator's live checks. Committed on `feat/p1.3a-docker-sandbox` (un-merged; the loop commits, the operator merges). As-built:
  ...
  - **PENDING the operator live check** (cannot be loop-verified — no Docker bring-up / LLM spend in-loop; acceptance = `make skeleton-run-docker`): **#1** that `on_event` actually fires over the container's WebSocket (the stale-docstring risk — the SDK source *has* a `WebSocketCallbackClient`, but confirm live); usage non-zero over the remote conversation; **pull-at-end** lands the file on the host and `git show ship-{run_id}:<file>` matches; the agent authenticates to OpenRouter from the container; **no orphan after a clean run** (`docker ps -a | grep agent-server` empty); and (P1.3b) exactly-once-ship + crash-resume over the container.
